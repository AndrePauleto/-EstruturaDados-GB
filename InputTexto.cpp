...
void InputTexto::desenhar()
{
	...
	txt.desenhar(gJanela.getLargura() / 2, gJanela.getAltura() / 2 - 100);
	...
	if (tamSelecao != 0)
	{
		...
		retan.y = (gJanela.getAltura() / 2) - 100 - retan.alt / 2;
	...
	int yCursor = (gJanela.getAltura() / 2) - 100 - altCursor / 2;
